...
int main() {
...

    {
        ...
        GGML_ASSERT(phoneme_vocab_size() == 178);
    }
    {
        // Codepoint → id mapping (from tokenizer.json model.vocab).
        GGML_ASSERT(kokoro_codepoint_to_id(U'h') == 50);
        GGML_ASSERT(kokoro_codepoint_to_id(U' ') == 16);
        GGML_ASSERT(kokoro_codepoint_to_id(0x02C8u) == 156); // ˈ primary stress
        GGML_ASSERT(kokoro_codepoint_to_id(0x0259u) == 83);  // ə schwa
        GGML_ASSERT(kokoro_codepoint_to_id(0x028Au) == 135); // ʊ
        GGML_ASSERT(kokoro_codepoint_to_id(0x2603u) == -1);  // ☃ not in vocab
    }
    {
    ...
    }
    {
        ...
        GGML_ASSERT(wrapped.size() == 4);
        GGML_ASSERT(wrapped.front() == KOKORO_PAD_ID);
        GGML_ASSERT(wrapped.back() == KOKORO_PAD_ID);
        GGML_ASSERT(wrapped[1] == 50 && wrapped[2] == 51);
    }
    {
        // Empty ids → just the two pad tokens.
        std::vector<int32_t> wrapped = wrap_input_ids({});
        GGML_ASSERT(wrapped.size() == 2);
        GGML_ASSERT(wrapped[0] == KOKORO_PAD_ID && wrapped[1] == KOKORO_PAD_ID);
    }
    {
        // Wrapping caps the phoneme run at 510 (512 with both pads).
        std::vector<int32_t> ids(2000, 50);
        std::vector<int32_t> wrapped = wrap_input_ids(ids);
        GGML_ASSERT(wrapped.size() == 512);
    }
    {
    ...
    }
    {
        ...
        GGML_ASSERT(kind == (espeak_available() ? KOKORO_G2P_ESPEAK : KOKORO_G2P_ASCII));
    }
    {
