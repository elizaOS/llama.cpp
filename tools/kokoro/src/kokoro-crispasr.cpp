...
static bool is_en_lang(const std::string& lang) {
    return lang == "en" || lang == "en-us" || lang == "en-gb" || lang == "en_us" || lang == "en_gb";
}

// Kokoro-82M was trained on hexgrad/misaki's English phoneme inventory, not raw
// espeak/CMU IPA. For English we therefore rewrite the IPA into misaki symbols
// (ʤ/ʧ/A/O/W/I/Y, no ː length marks) before it reaches kokoro_phonemes_to_ids,
// which otherwise silently drops the multi-codepoint sequences. British voices
// map oʊ/əʊ → Q; everything else uses the American table.
static void normalize_en_to_misaki(const std::string& lang, std::string& ipa) {
    if (!is_en_lang(lang))
        return;
    const bool gb = lang == "en-gb" || lang == "en_gb";
    ipa = crispasr::ipa_to_misaki(ipa, gb);
}
...
bool phonemize_cached(kokoro_context* ctx, const std::string& lang, const std::string& text, std::string& out) {
    ...
    if (builtin_ok && !out.empty()) {
        ...
        normalize_en_to_misaki(lang, out);
