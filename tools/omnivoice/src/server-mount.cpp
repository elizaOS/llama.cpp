...
//
// server-mount.cpp — committed implementation of the OmniVoice
// `/v1/audio/speech` HTTP route. Previously the preset-aware request
// parser + handler lived inline in `tools/server/server.cpp` (inside a
// large `#ifdef ELIZA_FUSE_OMNIVOICE` block). Before W3-3 the same code
// was injected at build time by the
// `packages/app-core/scripts/kernel-patches/server-omnivoice-route.mjs`
// patcher.
//
// H2.c moved both copies into committed source. `server.cpp` now only
// includes `server-mount.h` and forwards the CLI overrides + handler
// registration.
//
// The implementation is unchanged from the previous inline copy other
// than:
//   - `g_model_path` / `g_codec_path` are now external (declared in
//     server-mount.h), not file-static, so `server.cpp` can write to
//     them from `main()`.
//   - Added explicit include of `server-http.h` (transitively visible
//     before via server.cpp's includes; required directly here).
...
// Storage for the externs declared in server-mount.h.
std::string g_model_path;
std::string g_codec_path;
