...
// Encode-only entry point used by the freeze pipeline (W3-3): runs the
...
//   *out_K       -> number of codebooks (== ov_codebook_count(), 8 for the
//                   shipped DAC-RVQ tokenizer).
//   *out_ref_T   -> number of token frames produced (= ceil(n_samples /
//                   hop_length_in_24k_samples)).
//
// On failure the *out_* pointers are zeroed and ov_last_error() carries
// a diagnostic. Requires a codec-loaded handle.
//
// NB: this entry is independent of ov_synthesize — it does NOT consume
// any TTS params. The freeze CLI calls it once per reference clip and
// persists the resulting tokens in the ELZ2-v2 voice-preset blob.
