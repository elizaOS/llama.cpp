...
void llama_model_qwen35::load_arch_hparams(llama_model_loader & ml) {
    ...
    // Mark recurrent layers. Primary discovery: tensor presence (works for our existing
    // Eliza-1 GGUFs that don't encode full_attention_interval). MTP layers (the last
    // `nextn_predict_layers`) are always dense attention-only and must be non-recurrent.
    {
        const uint32_t n_main = hparams.n_layer - hparams.nextn_predict_layers;
        for (uint32_t i = 0; i < hparams.n_layer; ++i) {
            char buf[256];
            snprintf(buf, sizeof(buf), "blk.%u.ssm_conv1d.weight", i);
            const bool has_ssm = (ml.get_tensor_meta(buf) != nullptr);
            hparams.recurrent_layer_arr[i] = (i < n_main) && has_ssm;
        }
    }
