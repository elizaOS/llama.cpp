...
llama_model_qwen35moe::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    // Pre-norm hidden state: used by the AR draft loop to seed the next MTP step (eliza-1 output extraction).
    cb(cur, "h_pre_norm", -1);
    res->t_h_pre_norm = cur;

    // Final norm; post-norm hidden state feeds both the LM head and the MTP seed below
